	...
	function reverie_scripts_and_styles() {
	  if (!is_admin()) {
	    ...
	    wp_register_script( 'script-js', get_template_directory_uri() . '/js/script.js', array( 'jquery' ), '', true );
	    	...
	    	// adding Foundation scripts file in the footer
	    	wp_register_script( 'script-min-js', get_template_directory_uri() . '/js/remote/built.min.js', array( 'jquery' ), '', true );
	   	}
	    global $is_IE;
	    if ($is_IE) {
	       wp_register_script ( 'html5shiv', "http://html5shiv.googlecode.com/svn/trunk/html5.js" , false, true);
	       wp_register_script ( 'rem', get_template_directory_uri() . '/js/rem.min.js' , array( 'jquery' ), '', true );
	    }
	    ...
	    */
	    ...
	    wp_enqueue_script( 'script-js' );
			...
			wp_enqueue_script( 'script-min-js' );
		}
	  }
	}
}
...
if( ! function_exists( 'reverie_ie_conditional ' ) ) {
	function reverie_ie_conditional( $tag, $handle ) {
		if ( 'reverie-ie-only' == $handle )
			$tag = '<!--[if lt IE 9]>' . "\n" . $tag .'<script src="//cdnjs.cloudflare.com/ajax/libs/html5shiv/3.6.2/html5shiv.js"></script>
